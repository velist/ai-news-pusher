...
class MultiNewsAggregator:
    ...
    def categorize_article(self, title, summary):
        ...
        # 投资并购关键词
        if any(word in combined for word in ['funding', 'investment', 'raise', 'billion', 'million', 'valuation', 'ipo', 'acquisition', 'merger']):
            return '投资并购', '💰'
        
        # 公司动态关键词
        elif any(word in combined for word in ['openai', 'google', 'microsoft', 'apple', 'meta', 'amazon', 'nvidia', 'anthropic']):
            return '公司动态', '🏢'
        
        # 技术突破关键词
        elif any(word in combined for word in ['breakthrough', 'research', 'model', 'algorithm', 'technology', 'innovation', 'development']):
            return '技术突破', '🚀'
    ...
    def aggregate_news(self, max_total=60):
        ...
        # 从GNews获取新闻
        print("📡 正在获取GNews新闻...")
        gnews_articles = self.get_gnews_articles(max_results=25)
        all_articles.extend(gnews_articles)
        print(f"✅ GNews获取到 {len(gnews_articles)} 条新闻")
        
        # 从NewsAPI获取新闻
        print("📡 正在获取NewsAPI新闻...")
        newsapi_articles = self.get_newsapi_articles(max_results=25)
...
def generate_main_page(articles):
    ...
    all_categories = ['热门', '公司动态', '技术突破', '行业应用', '投资并购', '政策监管']
...
def generate_article_page(article):
    ...
    beijing_tz = timezone(timedelta(hours=8))
    try:
        if article['publishedAt'].endswith('Z'):
            dt = datetime.fromisoformat(article['publishedAt'][:-1] + '+00:00')
        else:
            dt = datetime.fromisoformat(article['publishedAt'])
        beijing_time = dt.astimezone(beijing_tz)
        formatted_time = beijing_time.strftime("%Y年%m月%d日 %H:%M")
    except:
        formatted_time = "未知时间"
...
<head>
    ...
    <style>
        ...
        .article-meta {{
            ...
            color: #666; font-size: 1.1em; margin-bottom: 20px;
        }}
    ...
    <div class="article-container">
        ...
        <div class="article-header">
            ...
            <div class="article-meta">
                ...
                <span class="article-time">{formatted_time}</span>
            </div>
        </div>
...
def main():
    ...
    print(f"📂 生成了 {len(processed_articles)} 个详情页面")
